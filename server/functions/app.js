/**
 * Express application for the Spire API.
 *
 * This module contains the platform-agnostic Express app and all route
 * definitions. It is consumed by two entry points:
 *   - index.js  -> Firebase Cloud Functions (onRequest)
 *   - lambda.js -> AWS Lambda (via serverless-http)
 *
 * Keeping the app here (with no firebase-functions dependency) lets the AWS
 * Lambda bundle stay small and avoids pulling Firebase into the Lambda runtime.
 */
...
apiRouter.get("/buildings", (req, res) => {
  ...
  console.info(`Using page and Limit: Query: ${req.query} Page: ${page}, Limt: ${limit}`);
...
apiRouter.get("/buildings/paginated", (req, res) => {
  ...
  console.info(`Buildings Paginated API - offset: ${offset}, limit: ${limit}`);
